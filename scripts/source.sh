...
expected_key="$LLAMA_CPP_REPO|$LLAMA_CPP_REF"
...
write_management_files() {
    local commit=$1
    printf 'native llama.cpp source directory\nroot=%s\n' "$ROOT_DIR" >"$managed_marker"
    printf '%s|%s|%s\n' "$LLAMA_CPP_REPO" "$LLAMA_CPP_REF" "$commit" >"$stamp.tmp"
...
if [[ -d "$SOURCE_DIR_ABS/.git" && "$SOURCE_UPDATE" == "0" ]] && read_stamp; then
    if [[ "$old_repo|$old_ref" == "$expected_key" ]] \
        ...
        verify_source_tree
        printf 'native llama.cpp source directory\nroot=%s\n' "$ROOT_DIR" >"$managed_marker"
        info "Reusing llama.cpp source at ${old_commit:0:12} ($LLAMA_CPP_REF)"
        exit 0
    fi
fi
...
if [[ "$OFFLINE" == "1" ]]; then
    ...
    elif offline_commit="$(git -C "$SOURCE_DIR_ABS" rev-parse --verify "${LLAMA_CPP_REF}^{commit}" 2>/dev/null)"; then
        :
    else
        die "OFFLINE=1 and ref '$LLAMA_CPP_REF' is not available in the local checkout"
    fi
    ...
    info "Offline mode: source ready at ${offline_commit:0:12} ($LLAMA_CPP_REF)"
    exit 0
fi

if [[ ! -d "$SOURCE_DIR_ABS/.git" ]]; then
    log "Initializing managed llama.cpp source checkout"
    mkdir -p "$SOURCE_DIR_ABS"
    printf 'native llama.cpp source directory\nroot=%s\n' "$ROOT_DIR" >"$managed_marker"
    git -C "$SOURCE_DIR_ABS" init --quiet
    git -C "$SOURCE_DIR_ABS" remote add origin "$LLAMA_CPP_REPO"
else
    ensure_clean_checkout
    printf 'native llama.cpp source directory\nroot=%s\n' "$ROOT_DIR" >"$managed_marker"
    if git -C "$SOURCE_DIR_ABS" remote get-url origin >/dev/null 2>&1; then
        git -C "$SOURCE_DIR_ABS" remote set-url origin "$LLAMA_CPP_REPO"
    else
        git -C "$SOURCE_DIR_ABS" remote add origin "$LLAMA_CPP_REPO"
    fi
fi

log "Fetching source ref $LLAMA_CPP_REF"
git -C "$SOURCE_DIR_ABS" fetch --force --depth=1 --no-tags origin "$LLAMA_CPP_REF"
...
info "Source ready: ${commit:0:12} ($LLAMA_CPP_REF)"
